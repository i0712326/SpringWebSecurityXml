...
define(function(require, exports, module){
	...
	var CartPanelAction = function(){
		...
		this.checkOutAll = function(that){
			...
			that.$el.find('#checkOutAllOk').hide();
			that.$el.find('#checkOutAllLoader').show();
			$.post(url,params,function(resp){
				that.$el.find('#checkOutAllLoader').hide();
				that.$el.find('#checkOutAllOk').show();
				$('#shopCartItems').text(0);
				$('#cart-total').text(0);
				$('.badge').text(0);
				$('#cart-sum').text(0);
				...
				$('#notiModal').modal('show');
				$('#notiModal').on('hidden.bs.modal', function () {
					window.location.href='#Home/Customer/'+id+'/History';
				});
			});
		}
		
		this.checkOutProduct = function(that){
			var data = that.model.toJSON();
